...
B<TOML>. The basic syntax is INI (C<key=value> lines and C<[section]>) but it
diverges more by allowing nested section using indentation and multiline values.
Round-tripping parsers exist for TOML.
